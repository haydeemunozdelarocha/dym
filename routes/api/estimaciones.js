...
router.post('/precio/:articuloid/:estimacionid/:conceptoid', function(req,res,err){
  ...
  var listaEstimaciones = 'UPDATE estimacion_articulo SET precio_unitario = '+precio+', importe = (esta_estimacion * '+precio+') WHERE articulo_id = '+articulo_id+';UPDATE estimaciones t4,(SELECT estimacion_id,(SUM(CASE WHEN categoria = "fletes" THEN importe ELSE 0 END)*.04) AS retencion,SUM(importe) AS subtotal,(SUM(importe)*.16) AS iva,(SUM(importe)-(SUM(CASE WHEN categoria = "fletes" THEN importe ELSE 0 END)*.04) + (SUM(importe)*.16)) AS total FROM estimacion_articulo WHERE estimacion_id = '+estimacion_id+') t1 SET t4.retencion = t1.retencion,t4.subtotal = t1.subtotal,t4.iva = t1.iva,t4.total = t1.total WHERE t4.estimaciones_id = t1.estimacion_id;';
