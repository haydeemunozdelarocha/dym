...
router.post('/recibos/resumen',isLoggedIn, function(req, res, next ){
  ...
  var recibosDia = "SELECT 'Flete' As Type, recibos.camion_id,acarreos_flete.cantidad,camiones.placas,proveedores.razon_social AS prov_flete, acarreos_flete.unidad,conceptos.nombre_concepto,zonas.nombre_zona,obras.nombre_obra, COUNT(*) AS viajes, SUM(cantidad) AS total_cantidad FROM recibos LEFT JOIN acarreos_flete ON acarreos_flete.recibo_id = recibos.recibo_id JOIN conceptos ON acarreos_flete.concepto_flete = conceptos.conceptos_id JOIN zonas ON zonas.zonas_id = recibos.zona_id JOIN camiones ON recibos.camion_id = camiones.camion_id JOIN obras ON recibos.obra_id = obras.obra_id JOIN proveedores ON proveedores.id = camiones.proveedor_id WHERE hora BETWEEN '"+date1+"' AND '"+date2+"' AND camiones.numero = "+numero+" AND recibos.obra_id = "+obra_id+" UNION SELECT 'Material', recibos.camion_id,acarreos_material.cantidad,camiones.placas,proveedores.razon_social AS prov_banco, acarreos_material.unidad,conceptos.nombre_concepto,zonas.nombre_zona,obras.nombre_obra, COUNT(*) AS viajes,SUM(cantidad) AS total_cantidad FROM recibos LEFT JOIN acarreos_material ON recibos.recibo_id = acarreos_material.recibo_id INNER JOIN conceptos ON acarreos_material.concepto_material = conceptos.conceptos_id LEFT JOIN camiones on recibos.camion_id = camiones.camion_id LEFT JOIN zonas ON zonas.zonas_id = recibos.zona_id LEFT JOIN materiales ON materiales.id = acarreos_material.material_id LEFT JOIN obras ON recibos.obra_id = obras.obra_id LEFT JOIN proveedores ON proveedores.id = acarreos_material.banco_id WHERE hora BETWEEN '"+date1+"' AND '"+date2+"' AND camiones.numero = "+numero+" AND recibos.obra_id = "+obra_id+" GROUP BY nombre_concepto;SELECT COUNT(recibos.camion_id) AS total_viajes FROM recibos LEFT JOIN camiones ON recibos.camion_id = camiones.camion_id LEFT JOIN proveedores AS proveedor_flete ON proveedor_flete.id = camiones.proveedor_id LEFT JOIN acarreos_flete ON recibos.recibo_id = acarreos_flete.recibo_id LEFT JOIN proveedores AS proveedor_banco ON proveedor_banco.id = acarreos_flete.banco_id LEFT JOIN conceptos ON acarreos_flete.concepto_flete = conceptos.conceptos_id LEFT JOIN zonas ON zonas.zonas_id = recibos.zona_id LEFT JOIN obras ON obras.obra_id = recibos.obra_id LEFT JOIN fletes ON acarreos_flete.flete_id = fletes.fletes_id WHERE hora BETWEEN '"+date1+"' AND '"+date2+"' AND numero = "+numero+" AND recibos.obra_id = "+obra_id+";";
  console.log(recibosDia)
  db.query(recibosDia, function(err, recibos){
    console.log(recibos[0][0])
    console.log(recibos[2])
    console.log(recibos[1])
    if(err) {
      res.send({message:'No se encontraron totales para esta obra.'})
      }
       else {
        ...
        res.render('recibosresumen',{recibos:recibos[0],acarreos:recibos[0],total:recibos[2],fecha:hora_recibo, message: ''})
        }
    }
  });
})
